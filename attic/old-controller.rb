...
ircName = 'ncontrol' + (`cat $OPENSHIFT_DATA_DIR/conf/controller_number`.chomp)
trigger = 'nc' + (`cat $OPENSHIFT_DATA_DIR/conf/controller_number`.chomp) + ':'
...
if $bots.any? {|h| h["userid"] == userid}
						...
						# bot is already running
                                        ...
                                        if $bots.any? {|h| h["userid"] == userid}
                                                ...
                                                pid = spawn($rvm_env_vars.merge({ "BOTPORT" => testPort.to_s, "MAGICKEY" => magicKey, "BOTUSERID" => userid}), "#{ENV['OPENSHIFT_REPO_DIR']}diy/bot/neuroBotSocketWrapper.rb")
			...
			magic_key = ""
        		#lookup magic key
                	$db.query("select magic_key from users where bot_userid='#{botid}' LIMIT 1").each do |row|
                      	...
                      	controller = `cat $OPENSHIFT_DATA_DIR/conf/controller_number`.chomp
...
#rvm_env_vars = { "GEM_HOME" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/gems/ruby-1.9.3-p125", "GEM_PATH" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/gems/ruby-1.9.3-p125:/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/gems/ruby-1.9.3-p125@global", "HISTFILE" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data/.bash_history", "IRBRC" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/rubies/ruby-1.9.3-p125/.irbrc", "MY_RUBY_HOME" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/rubies/ruby-1.9.3-p125", "PATH" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/gems/ruby-1.9.3-p125/bin:/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/gems/ruby-1.9.3-p125@global/bin:/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/rubies/ruby-1.9.3-p125/bin:/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/bin:/usr/libexec/openshift/cartridges/diy-0.1/info/bin/:/usr/libexec/openshift/cartridges/abstract-httpd/info/bin/:/usr/libexec/openshift/cartridges/abstract/info/bin/:/bin:/usr/bin:/sbin:/usr/sbin", "RUBY_VERSION" => "ruby-1.9.3-p125", "rvm_bin_path" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm/bin", "rvm_env_string" => "ruby-1.9.3-p125", "rvm_path" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data//.rvm", "rvm_prefix" => "/var/lib/openshift/445b548ca10d4d3a92a0b73cce110875/app-root/data/", "rvm_ruby_string" => "ruby-1.9.3-p125", "rvm_version" => "1.12.2-openshift (master)" }
...
$ircSocket = Ponder::Thaum.new do |config|
  ...
  config.port   = 40000 
  config.password = 'somethingspecial'
end

$ircSocket.on :connect do
  $ircSocket.join '#neurobots'
end

$ircSocket.on :channel, /#{trigger} status/ do |event_data|
  $ircSocket.message event_data[:channel], "Online: Bots active #{$bots.count}" 
end
...
$ircSocket.on :channel, /#{trigger} start/ do |event_data|
	botid = event_data[:message].gsub( /#{trigger} start /, '')
	magic_key = ""	
	#lookup magic key
	$db.query("select magic_key from users where bot_userid='#{botid}' LIMIT 1").each do |row|
		magic_key = row[0]
	...
	$db.query("update users set controller='#{controller}' where bot_userid='#{botid}'")
	irc_start_bot(botid, magic_key)
        $ircSocket.message event_data[:channel], "Starting #{botid} #{magic_key} #{controller}"
